...
function toCelsius(temperatures: number[]): number[] {
  // write your code here...
  const convertedTemps = temperatures.map((x) => (x - 32) * (5 / 9));

  return convertedTemps; // replace empty array with what you see is fit
}
...
function hottestDays(temperatures: number[], threshold: number): number[] {
  // write your code here...
  temperatures = temperatures.filter((x) => x > threshold);
  return temperatures; // replace empty array with what you see is fit
}
...
function logHottestDays(temperatures: number[], threshold: number): void {
  ...
  const convertedArr = toCelsius(hotDays);
  logger(convertedArr);
}
